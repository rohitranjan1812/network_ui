"""
Visualization API for graph rendering and interaction.
Implements Spec 3:   Graph Visualization requirements.
"""
...
from ...api.graph_engine import GraphEngineAPI
...
class VisualizationAPI: 
    ...
    def __init__(self): 
        ...
        self.renderer = None
        self.visual_mapping = None  # VisualMapper()  # This will be initialized lazily
        self.interaction_manager = InteractionManager()
        self.graph_engine = None  # Will be initialized lazily

  # Current visualization state
        self._current_highlights:   List[str] = []
  ...
  # Apply layout if requested
                ...
                if layout_algorithm: 
  # Assuming LayoutParams is available or needs to be imported
  # from ..layouts import LayoutParams
  ...
  # graph_data = layout.apply_layout(graph_data)
                    pass  # Placeholder for layout application

  # Apply visual mappings
                if data.get('visual_mappings'): 
  # Assuming VisualMapper is available or needs to be imported
  # from ..visual_mapping import VisualMapper
  # self.visual_mapping.apply_mappings(graph_data, data['visual_mappings'])
                    pass  # Placeholder for visual mapping application

  # Render the graph
                self._current_highlights = list(highlights)  # Convert set to list for compatibility
  ...
  # Validate algorithm
                valid_algorithms = ['force_directed', 'hierarchical', 'circular', 'grid', 'random']
                if algorithm not in valid_algorithms: 
                    return jsonify({'error':  f'Invalid layout algorithm:   {algorithm}. Valid algorithms are:   {valid_algorithms}'}), 400
  ...
  # Assuming create_layout is available or needs to be imported
  ...
  #         original_node.position = updated_node.position.copy()
                ...
                return jsonify({
                    ...
                    'algorithm':  algorithm,
                    'timestamp':  datetime.now().isoformat(),
                    'node_count':  len(graph_data.nodes),  # Placeholder for node count
                    'edge_count':  len(graph_data.edges),  # Placeholder for edge count
  ...
  # Validate mappings
                if not self._validate_visual_mappings(mappings): 
  ...
  # Update current highlights
                self._current_highlights = list(set(node_ids + edge_ids))  # Convert set to list for compatibility
        ...
        def update_viewport(): 
            """Update viewport settings (zoom, pan, etc.)."""
            try: 
                data = request.get_json()
                if not data: 
                    return jsonify({'error':  'No data provided'}), 400
  ...
  # Update viewport state
                if 'x' in data: 
                    self._viewport_state['x'] = data['x']
                if 'y' in data: 
                    self._viewport_state['y'] = data['y']
                if 'zoom' in data: 
                    self._viewport_state['zoom'] = data['zoom']

  # Note:   set_viewport method doesn't exist in renderer
        ...
        def get_configuration(): 
            ...
            try: 
                return jsonify({
                    ...
                    'viewport':  self._viewport_state,
                    'current_highlights':  self._current_highlights,
  ...
  # Process interaction
                result = self.interaction_manager.handle_interaction(data)
  ...
  # Store visibility in edge attributes
    ...
    def _validate_visual_mappings(self, mappings:   Dict[str, Any]) -> bool: 
        """
        Validate the visual mapping configuration.
        """
        try: 
  # Check if mappings structure is valid
            if not isinstance(mappings, dict): 
                return False

  # Check for nodes and edges sections
            nodes_mappings = mappings.get('nodes', {})
            edges_mappings = mappings.get('edges', {})

  # Validate node mappings
            for mapping_name, mapping_config in nodes_mappings.items(): 
                if not isinstance(mapping_config, dict): 
                    return False

  # Check required fields for each mapping
                if 'attribute' not in mapping_config or 'mapping_type' not in mapping_config: 
                    return False

  # Validate mapping type
                valid_types = ['linear', 'categorical', 'logarithmic', 'ordinal']
                if mapping_config['mapping_type'] not in valid_types: 
                    return False

  # Validate edge mappings (similar structure)
            for mapping_name, mapping_config in edges_mappings.items(): 
                if not isinstance(mapping_config, dict): 
                    return False
            ...
            self.logger.error(f"Error validating visual mappings:   {e}")
            return False
